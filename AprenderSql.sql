...
---E2-(create table - sp_tables - sp_columns - drop table);
...
--E3-(Insertar y recuperar registros de una tabla (insert into - select));
if object_id('agenda') is not null
...
---E5-(Recuperar algunos campos (select));
if OBJECT_ID('Peliculas') is not null
...
---E6-(Recuperar algunos registros (where))
if OBJECT_ID('agenda') is not null
	drop table agenda;
...
--Seleccione el registro Donde esta "Marisa";
select * from agenda where nombre = 'marisa';

---seleciona los nombres y apellidos que contenga "lopez"
...
--selecciona los articulos que tengan por nombre "impresora"
...
---Borra los articulos donde el codigo sea <>4
delete from articulos where codigo<>4;

---Muestra la agenda
select * from articulos;

---E9-(Actualizar registros (update))
if object_id('agenda') is not null
	drop table agenda;
...
---cambia "paenza" por "Adrian Paenza"
update libros set autor='Adrian Paenza' where autor='Paenza';
...
--E13-(Campo con atributo Identity)
if object_id('medicamentos') is not null
	drop table medicamentos;
...
--E14-(Otras características del atributo Identity)
if object_id('medicamentos') is not null
	drop table medicamentos;
...
---E20-(Ingresar algunos campos (insert into))
if object_id('cuentas') is not null
	drop table cuentas;
...
---E21-(Valores por defecto (default))
...
---E22-(Columnas calculadas (operadores aritméticos y de concatenación))
if object_id ('articulos') is not null
	drop table articulos;
...
---E27-(Funciones para el uso de fechas y horas)
if object_id ('empleados') is not null
	drop table empleados;
...
---E28-(Ordenar registros (order by))
...
---E29-(Operadores lógicos ( and - or - not))
if object_id('medicamentos') is not null
	drop table medicamentos;
...
---E30-(Otros operadores relacionales (is null))
if object_id('peliculas') is not null
	drop table peliculas;
...
---E31-(Otros operadores relacionales (between))
if object_id('visitas') is not null
	drop table visitas;
...
---E32-(Otros operadores relacionales (in))
if object_id('medicamentos') is not null
	drop table medicamentos;
...
---E33-(Búsqueda de patrones (like - not like))
if object_id('empleados') is not null
	drop table empleados;
...
---E35-(Contar registros (count_big))
if object_id('medicamentos') is not null
	drop table medicamentos;
...
---E36-(Funciones de agrupamiento (count - sum - min - max - avg))
...
---E37-(Agrupar registros (group by))
if object_id('visitantes') is not null
	drop table visitantes;
...
---E39-(Modificador del group by (with rollup))
if object_id('clientes') is not null
	drop table clientes;
...
---E40-(Modificador del group by (with cube))
...
---E42-(Cláusulas compute y compute by)
...
---E43-(Registros duplicados (distinct))
if object_id('clientes') is not null
	drop table clientes;
...
---E49-(Deshabilitar restricciones (with check - nocheck))
...
---E53-(Eliminar restricciones (alter table - drop))
if object_id('vehiculos') is not null
drop table vehiculos;
...
---E54-(Crear y asociar reglas (create rule - sp_bindrule))
...
---E55-(Eliminar y dasasociar reglas (sp_unbindrule - drop rule))
if object_id('vehiculos') is not null
drop table vehiculos;
if object_id ('RG_patente_patron') is not null
drop rule RG_patente_patron;
...
---E57-(Valores predeterminados (create default))
if object_id ('clientes') is not null
drop table clientes;
...
---E58-(Desasociar y eliminar valores predeterminados)
if object_id ('libros') is not null
	drop table libros;
...
---E66-(Combinación interna (inner join))
...
---E67-(Combinación externa izquierda (left join))
if (object_id('clientes')) is not null
	drop table clientes;
if (object_id('provincias')) is not null
	drop table provincias;
...
---E68-(Combinación externa derecha (right join))
if (object_id('clientes')) is not null
	drop table clientes;
if (object_id('provincias')) is not null
	drop table provincias;
...
---E69-(Combinación externa completa (full join))
...
---E70-(Combinaciones cruzadas (cross join))
...
---E72-(Combinaciones y funciones de agrupamiento)
if object_id('visitantes') is not null
drop table visitantes;
...
---E73-(Combinación de más de dos tablas)
